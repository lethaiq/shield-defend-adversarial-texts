...
class BertClassifierDARTS(nn.Module):
    def __init__(self, 
        ...
        self.bert_layer = AutoModel.from_pretrained(model_type)
            ...
            if self.darts:
                self.heads = nn.ModuleList([
                        nn.ModuleList([
                            ...
                            nn.Sequential(nn.Linear(self.feature_dim, self.feature_dim), nn.ReLU(), nn.Linear(self.feature_dim, self.output_dim)),
                            nn.Sequential(nn.Linear(self.feature_dim, self.feature_dim), nn.ReLU(), nn.Linear(self.feature_dim, self.feature_dim), nn.ReLU(), nn.Linear(self.feature_dim, self.output_dim)),
                ...
                self.heads = nn.ModuleList([nn.ModuleList([nn.Sequential(nn.Linear(self.feature_dim, self.output_dim))]) for i in range(self.N)])

            self.switch_train = torch.ones(self.N).to(self.device).float()
            self.switch = nn.Linear(self.N*self.output_dim+self.feature_dim, self.N)
            self.probs = self.switch_train
        else:
    ...
    def forward_from_embedding(self, emb, attn_masks):
        try:
            ...
            cont_reps, _ = self.bert_layer(inputs_embeds=emb, attention_mask=attn_masks)
            pred = cont_reps[:, 0]
            ...
            for i in range(len(self.heads)):
                ...
                if self.darts:
                    if not self.inference:
                        ...
                        scores = [(layer[j](pred)*controls[j]).unsqueeze(0) for j in range(len(layer))]
